...
class ProductImporter {
    ...
    private function process_variations($product, $variations) {
        ...
        $this->create_all_variations($product);

        // Get available variations and update them
        ...
        foreach ($available_variations as $available_variation) {
            if (isset($variations[$variation_index])) {
                ...
                $this->update_variation(
                    $available_variation['variation_id'],
                    $variation_data
                );
                $variation_index++;
            }
    ...
    private function update_variation($variation_id, $variation_data) {
        try {
            ...
            $sku = sanitize_text_field($variation_data[6]); // SKU column
